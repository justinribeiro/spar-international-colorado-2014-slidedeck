                ...
                ws.connection.onmessage = function(event){
                    ...
                    if (options.topic) {
                      if (event.data !== undefined) {
                        var topicIncoming = $.parseJSON(event.data);
                        ws.topic( topicIncoming.topic ).publish( topicIncoming.topic, topicIncoming.message  );
                      }
                    } else {
                        ws.topic( "websocket.onMessage" ).publish( event.data );
                    }
